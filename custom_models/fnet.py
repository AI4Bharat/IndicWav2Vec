...
@register_model("wav2vec2_fnet", dataclass=Wav2Vec2Config)
...
class TransformerEncoder(nn.Module):
    ...
    def extract_features(self, x, padding_mask=None):
        # TODO check if this is required for fnet
        if padding_mask is not None:
            x[padding_mask] = 0
...
class FnetSentenceEncoderLayer(nn.Module):
    """
    Implements a Fnet Encoder Layer used in BERT/XLM style pre-trained
    models.
    """
    ...
    def __init__(
        ...
        self.fnet_layer_norm = LayerNorm(self.embedding_dim)
    ...
    def forward(
        ...
        """
        LayerNorm is applied either before or after the fourier/ffn
        modules similar to the original Transformer imlementation.
        """
        residual = x

        if self.layer_norm_first:
            x = self.fnet_layer_norm(x)
