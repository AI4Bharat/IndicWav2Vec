...
indiccorp_path = "/nlsasfs/home/ai4bharat/manidl/speechteam/indic-corp-os"
...
# %%
def clean_and_stats(input, lang, allowed_chars_dict, set_digits, ssplit=False):
    '''
    ...
    no_puncs = remove.sub(" ", no_bracs).strip()

    # Normalize and num2word
    norm_input = normalize_sentence(no_puncs, lang_code)
...
# %%
def get_corpus_stats(corpus_path, dict_path, lang):
    ...
    data_and_stats = Parallel(n_jobs=-1, backend='multiprocessing')(delayed(clean_and_stats)(p, lang, allowed, set_digits) for p in tqdm(corpus))
    print('\tDone Processing!')
    return allowed, data_and_stats

# %%
def filter_and_clean(dataframe, valid_chars, strict=True):
...
# sent_list = get_sents(corpus_path, LANG)
# data_and_stats = get_corpus_stats(corpus_path, dict_path, LANG)
...
if LANG == '':
    ...
    if save:
        ...
        ].plot(kind='box', title=f'[{lang}] Word Level', subplots=True)[0].get_figure().savefig(f'{plots_dir}/word_level_boxplot.jpg')
        ...
        ].plot(kind='box', title=f'[{lang}] Character Level', subplots=True)[0].get_figure().savefig(f'{plots_dir}/character_level_boxplot.jpg')
        df_stats.to_csv(f'{plots_dir}/stats.tsv', sep='\t')
