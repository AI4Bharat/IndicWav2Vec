...
class TempAudioPretrainingTask(FairseqTask):
    ...
    # https://github.com/pytorch/fairseq/blob/1bba712622b8ae4efb3eb793a8a40da386fe11d0/fairseq/tasks/multilingual_masked_lm.py#L141
    ...
    def load_dataset(
        ...
        for lang in _splits:
            ...
            if task_cfg.labels:
                label_path = os.path.join(data_path, f"{lang}.{task_cfg.labels}")
                skipped_indices = getattr(dataset, "skipped_indices", set())
                with open(label_path, "r") as f:
                    labels = [
                        line for i, line in enumerate(f) if i not in skipped_indices
                    ]
        ...
        if len(_splits) == 1:
            ...
            size_ratio = (sample_probs * datasets_sizes.sum()) / datasets_sizes
            sr_str = str(
